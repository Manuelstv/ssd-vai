...
class PascalVOCDataset(Dataset):
    ...
    def __init__(self, data_folder, split, keep_difficult=False):
        ...
        self.image_dir = '/home/manuelveras/dataset/train/images'
    ...
    def __getitem__(self, i):
        ...
        for obj in root.findall('object'):
            ...
            xmin = int(bbox.find('x_center').text) - int(float(bbox.find('width').text))/2
            ymin = int(bbox.find('y_center').text) - int(bbox.find('height').text)/2
            xmax = int(bbox.find('x_center').text) + int(float(bbox.find('width').text))/2
            ymax = int(bbox.find('y_center').text) + int(bbox.find('height').text)/2
